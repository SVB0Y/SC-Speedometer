...
void PluginInit() {
  g_Module.ScriptInfo.SetAuthor("Nero & incognico, Modified by SV BOY");
  ...
  @g_pSpeedThinkFunc = g_Scheduler.SetInterval("speedThink", 0.06f);
}
...
HookReturnCode ClientSay(SayParameters@ pParams) {
  ...
  if (pArguments.ArgC() == 1) {
    if (pArguments.Arg(0) == "dev") {
      pParams.ShouldHide = true;
      string szSteamId = g_EngineFuncs.GetPlayerAuthId(pPlayer.edict());

      if (g_PlayerSpeed.exists(szSteamId)) {
        g_PlayerFuncs.ClientPrint(pPlayer, HUD_PRINTTALK, "Developer disabled\n");
        removeSpeedometer(pPlayer);
      }
      else {
        g_PlayerFuncs.ClientPrint(pPlayer, HUD_PRINTTALK, "Developer enabled\n");
...
void speedMsg(CBasePlayer@ pPlayer, const string szSteamId) {
  ...
  float speedh, realSpeedH, origin_x, origin_y, origin_z, speedy, currentTime;
  ...
  speedy = sqrt( pow( velocity.z, 2.0 ) ); 
  origin_x = (origin.x);
  origin_y = (origin.y);
  origin_z = (origin.z);
  ...
  // Show SpeedoMeter
  g_PlayerFuncs.HudMessage(pPlayer, txtPrms, string(int(speedh)) + "\n" + string(int(speedy)) + "\n\n" + string(int(origin_x)) + " " + string(int(origin_y)) + " " + string(int(origin_z)) );
}
